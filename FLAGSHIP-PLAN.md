...
- **Fill batch 2 (10-11 Jun 2026): 30 more inserted**, bringing the queue to 96 of 109 planned. **13 slugs remain unwritten and reserved** (do not let the autopilot or another session take them for different content): ev-total-cost-of-ownership, charging-your-ev-from-solar, when-your-fixed-energy-deal-ends, is-it-worth-switching-energy-2026, using-your-in-home-display, economy-10-explained, why-did-my-direct-debit-go-up, octopus-agile-tracker-tariffs-explained, energy-saving-in-a-shared-house, energy-saving-in-a-flat, save-energy-in-the-kitchen, save-energy-in-the-bathroom, energy-saving-with-a-baby. Their titles, clusters and pubdate slots are in `.flagship/plan.json`. To finish: trim `.flagship/workflow_fill43.js`'s INPUT to the missing slugs (compare plan.json against `.flagship/articles/*.json`), run it via the Workflow tool, then `python3 .flagship/assemble_fill.py` (insert-only, idempotent), build, verify, push.
